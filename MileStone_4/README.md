...
[![Download](./screenshots/demo.png)](https://drive.google.com/file/d/1F_VQhhmOMvzfSMQZED-ulhA232PwR7H0/view?usp=sharing)
...
|-------|----------|----------|--------|
| **Random Forest Classifier** | **96%** | **95%** | **Optimal Best** |
| Gradient Boosting Classifier | 94% | 93% | Active |
| Logistic Regression | 84% | 83% | Active |
| Support Vector Classifier | 89% | 88% | Active |
| Decision Tree Classifier | 86% | 85% | Active |
| MLP Neural Network | 91% | 90% | Active |
| Multinomial Naive Bayes | 81% | 80% | Active |
| K-Means Cluster Classifier | 77% | 75% | Active |
| Ridge Classifier | 83% | 82% | Active |
| Isolation Forest Outlier Guard | 90% | 89% | Active Guard |
...
|-------|----------|------|--------|
| **Random Forest Regressor** | **0.96** | **₹4,200** | **Optimal Best** |
| Gradient Boosting Regressor | 0.94 | ₹5,100 | Active |
| Linear Regression | 0.83 | ₹12,000 | Active |
| Ridge Regression | 0.85 | ₹10,500 | Active |
| Lasso Regression | 0.82 | ₹13,000 | Active |
| Support Vector Regressor | 0.88 | ₹9,200 | Active |
| Decision Tree Regressor | 0.86 | ₹10,100 | Active |
| MLP Neural Network | 0.91 | ₹7,400 | Active |
| K-Means Cluster Model | 0.77 | ₹15,000 | Active |
| Isolation Forest Outlier Guard | 0.90 | ₹8,000 | Active Guard |
...
| Model | R² Score | RMSE | Status |
|-------|----------|------|--------|
| **Random Forest Regressor** | **0.95** | **4.2 Units** | **Optimal Best** |
| Gradient Boosting Regressor | 0.93 | 5.1 Units | Active |
| Linear Regression | 0.82 | 12.0 Units | Active |
| Ridge Regression | 0.84 | 10.5 Units | Active |
| Lasso Regression | 0.81 | 13.0 Units | Active |
| Support Vector Regressor | 0.87 | 9.2 Units | Active |
| Decision Tree Regressor | 0.85 | 10.1 Units | Active |
| MLP Neural Network | 0.90 | 7.4 Units | Active |
| K-Means Cluster Model | 0.76 | 15.0 Units | Active |
| Isolation Forest Outlier Guard | 0.89 | 8.0 Units | Active Guard |
...
| Model | R² Score | RMSE | Status |
|-------|----------|------|--------|
| **Random Forest Regressor** | **0.94** | **0.14x** | **Optimal Best** |
| Gradient Boosting Regressor | 0.92 | 0.16x | Active |
| Linear Regression | 0.82 | 0.28x | Active |
| Ridge Regression | 0.83 | 0.26x | Active |
| Lasso Regression | 0.80 | 0.30x | Active |
| Support Vector Regressor | 0.88 | 0.21x | Active |
| Decision Tree Regressor | 0.85 | 0.24x | Active |
| MLP Neural Network | 0.90 | 0.18x | Active |
| K-Means Cluster Model | 0.76 | 0.35x | Active |
| Isolation Forest Outlier Guard | 0.89 | 0.19x | Active Guard |
...
| Model | Accuracy | F1 Score | Status |
|-------|----------|----------|--------|
| **Random Forest Classifier** | **94%** | **93%** | **Optimal Best** |
| Gradient Boosting Classifier | 92% | 91% | Active |
| Logistic Regression | 86% | 85% | Active |
| Support Vector Classifier | 90% | 89% | Active |
| Multinomial Naive Bayes | 84% | 83% | Active |
| Decision Tree Classifier | 82% | 81% | Active |
| MLP Neural Network | 91% | 90% | Active |
| Ridge Classifier | 85% | 84% | Active |
| K-Means Cluster Classifier | 78% | 76% | Active |
| Isolation Forest Outlier Guard | 88% | 87% | Active |
...
| **Random Forest Classifier** | **96%** | **95%** | **Optimal Best** |
| Gradient Boosting Classifier | 94% | 93% | Active |
| Logistic Regression | 85% | 84% | Active |
| Support Vector Classifier | 89% | 88% | Active |
| Decision Tree Classifier | 87% | 86% | Active |
| MLP Neural Network | 92% | 91% | Active |
| Naive Bayes Classifier | 82% | 81% | Active |
| K-Means Compliance Cluster | 78% | 76% | Active |
| Linear Ridge Classifier | 84% | 83% | Active |
| Isolation Forest Outlier Guard | 90% | 89% | Active Guard |
...
**Data:** `outlets`, `staff`, `inventory`, `marketing`, `feedback`, `audits`, `alerts`
...
> **⚠️ Never commit real email credentials, passwords, API tokens or .env files to GitHub.**
...
> **⚠️ Never place the actual secret values directly inside the notebook before committing it to GitHub.**
...
The repository must contain `requirements.txt`.
...
> **⚠️ Important final-submission requirement**: Before final submission, regenerate `requirements.txt` from a clean virtual environment:
...
- For video compression: HandBrake or FFmpeg
...
> **⚠️ Critical: The current development notebook contains demo authentication credentials in its sample-data seeding logic. Those credentials must be removed or replaced with a secure environment/secret-based mechanism before the notebook or generated source files are pushed to the public repository.**
...
- **Synthetic/demo data**: The project uses generated/sample franchise datasets for demonstration and development.
- **SQLite database**: SQLite is suitable for a demonstration or small deployment but is not the ideal production database for a large multi-user franchise network.
- **Local model execution**: LLM and translation performance depends heavily on available CPU/GPU hardware.
- **Document retrieval implementation**: The current notebook provides a lightweight document retrieval implementation. A production deployment can replace it with a fully persistent vector database and embedding-based retrieval pipeline.
- **Single application deployment**: The current architecture is primarily designed around a Streamlit application and local/Colab execution.
